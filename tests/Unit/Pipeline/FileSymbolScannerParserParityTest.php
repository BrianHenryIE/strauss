...
PHP,
    ...
    private function assertScannerParity(string $contents, string $sourcePath): void
    {
        ...
        $legacySymbols = $legacyScanner->scanString(
            $contents,
            new File(
                $sourcePath,
                basename($sourcePath),
        ...
        $modernSymbols = $modernScanner->scanString(
            $contents,
            new File(
                $sourcePath . '.new',
                basename($sourcePath . '.new'),
                $targetPath
            )
        );

        $this->assertSame(
            json_encode($this->snapshotSymbols($legacySymbols)),
            json_encode($this->snapshotSymbols($modernSymbols))
        );
    }
