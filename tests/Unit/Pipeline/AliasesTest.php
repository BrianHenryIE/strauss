...
function autoloadAliases( $classname ): void {
  $includeFilePath = __DIR__ . '/autoload_alias.php';
  switch( $classname ) {
    case 'Foo\\Bar\\Baz':
      $includeFile = '<?php namespace Foo\Bar; class Baz extends \Baz\Foo\Bar\Baz {}';
      file_put_contents($includeFilePath, $includeFile);
      include $includeFilePath;
      file_exists($includeFilePath) && unlink($includeFilePath);
      break;
...
spl_autoload_register( '\Baz\autoloadAliases' );
...
EOD;
    ...
    public function test_functions(): void
    {
        ...
        $config->expects('getVendorDirectory')->twice()->andReturn('vendor/');
        $config->expects('getTargetDirectory')->once()->andReturn('vendor-prefixed/');
        $config->expects('getNamespacePrefix')->once()->andReturn('Baz\\');
