...
EOD;
        ...
        $this->assertEqualsPaths(
            $this->getFileSystem()->normalizePath($this->getFileSystem()->makeAbsolute($projectDir . '/target_directory/')),
...
EOD;
        ...
        $this->assertEqualsPaths(
            $this->getFileSystem()->normalizePath($this->getFileSystem()->makeAbsolute($projectDir . '/vendor-prefixed/')),
            $sut->getAbsoluteTargetDirectory()
        );
    }
...
EOD;
        ...
        $this->assertEqualsPaths(
            $this->getFileSystem()->normalizePath($this->getFileSystem()->makeAbsolute($projectDir . '/dep_directory/')),
            $sut->getAbsoluteTargetDirectory()
        );
...
EOD;
    ...
    public function test_exclude_from_string_rename_default_empty(): void
    {
        $composerExtraStraussJson = <<<'EOD'
{
 "extra":{
...
EOD;
        ...
        try {
            ...
            $composer = Factory::create(new NullIO(), $tmpfname);
            $sut = new StraussConfig($composer);
            $this->assertSame([], $sut->getExcludeFunctionsFromStringRenaming());
        } finally {
            unlink($tmpfname);
        }
    }

    public function test_exclude_from_string_rename_functions(): void
    {
        $composerExtraStraussJson = <<<'EOD'
{
 "extra":{
...
EOD;
        ...
        try {
            ...
            $composer = Factory::create(new NullIO(), $tmpfname);
            $sut = new StraussConfig($composer);
            $this->assertSame(['value', 'when'], $sut->getExcludeFunctionsFromStringRenaming());
        } finally {
            unlink($tmpfname);
        }
    }
